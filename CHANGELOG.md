...
- 功能优化：重构用户登录逻辑，支持双模式认证（手机号+短信验证码 / 用户名+密码）
- 功能新增：新增服务条款、隐私政策和免责声明页面，完善用户协议体系
- 功能优化：优化协议文件格式，使用 Markdown 渲染，支持标题层级和关键内容加粗
- 功能新增：充值页面新增服务条款提示，用户充值前需阅读并同意相关条款
- 功能优化：移除登录接口的滑动验证码校验，简化登录流程（验证码仅用于短信发送验证）
